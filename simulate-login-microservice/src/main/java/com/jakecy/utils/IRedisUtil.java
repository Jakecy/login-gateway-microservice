...
//    }
    ...
    public void setRedisTemplate(RedisTemplate redisTemplate) {
        ...
        logger.info("setRedisTemplate init redis util complete.");
    ...
    public void setTemplate(RedisTemplate redisTemplate) {
        ...
        redisTemplate.setKeySerializer(stringSerializer);
        redisTemplate.setHashKeySerializer(stringSerializer);
        logger.info("setTemplate init redis util complete.");
        this.template = redisTemplate;
    }
	/**
...
//		logger.info("{}集合中添加元素{}", key, value);
	...
	public void delAndKey(String key){
	    template.delete(key);
    }
...
//		logger.info("获取对象 {} : {}", key, result);
	...
	private BoundValueOperations<String, Object> str(String key) {
		return template.boundValueOps(key);
	}

	private BoundHashOperations<String, String, Object> h(String key) {
		return template.boundHashOps(key);
	}

	private BoundZSetOperations<String, Object> z(String key) {
		return template.boundZSetOps(key);
	}

	private BoundSetOperations<String, Object> s(String key) {
		return template.boundSetOps(key);
	}
	
	/**
	 * @Title: generateNum   
	 * @Description: 生成制定规则的编号  
	 * @param prefix  前缀 如 SQCZ、
	 * @param date_formate 时间格式  yyyyMMddHHmm
	 * @param num_formate 后几位数字格式化  000000
	 * @return String  SQCZ+时间戳(年月日时分秒)+六位顺序编号 
	 * @throws
	 */
	public String generateNum(String prefix,String date_formate,String num_formate) {
		...
		sBuilder.append(this.getFormate(prefix + "_"+sdfDay.format(new Date()),num_formate));
		return sBuilder.toString();
	}
	
	private String getFormate(String key,String num_formate) {
		Long incr = incr(key);
		if (incr != null && incr == 1) { //第一次生成
			expire(key, 24*60*60); //24小时后失效
		}
		DecimalFormat df = new DecimalFormat(num_formate);
		return df.format(incr.intValue());
	}
